...
converted_users_df = (sales_df.select("email").drop_duplicates(["email"]).withColumn("converted", lit(True))
                     )
...
# TODO
abandoned_items_df = (abandoned_carts_df.withColumn("items", explode("cart")).select("items").groupBy("items").count()
                     )
